...
const generateCode = () => {
  ...
  components.forEach(comp => {
    if (comp.type === 'fade') {
      html += `<section style="pointer-events: none; font-size: 0">\n`
      
      html += `  <svg
    style="background-image: url('${comp.props.imageUrl}'); background-size: 100%; background-repeat: no-repeat; display: block; line-height: 0; transform: scale(1); margin-top: -1px"
    viewBox="0 0 ${comp.props.imageWidth} ${comp.props.imageHeight}">\n`

      // 添加动画定义
      if (comp.props.hotspots && comp.props.hotspots.length > 0) {
        comp.props.hotspots.forEach(hotspot => {
          if (hotspot.type === 'animate' && hotspot.animateCode) {
            html += hotspot.animateCode + '\n'
          }
        })
      }
      ...
      if (comp.props.hotspots && comp.props.hotspots.length > 0) {
        comp.props.hotspots.forEach(hotspot => {
          if (hotspot.type === 'link') {
            // 链接跳转热区
            html += `    <a href="${hotspot.link}" target="_blank" style="pointer-events: visible;">
      <rect x="${hotspot.x}" y="${hotspot.y}" width="${hotspot.width}" height="${hotspot.height}" fill="transparent"/>
    </a>\n`
          } else {
            // 动画触发热区
            html += `    <rect 
      ...
      fill="transparent"
      opacity="0"
      begin="${hotspot.triggerEvent || 'click'}"/>\n`
          }
        })
      }
